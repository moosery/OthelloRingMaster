...
static void RunStatsListenerJob(uint32_t /*thdIdx*/, PSolveContext pCtx)
{
    ...
    */
    ...
    if (wsaResult != 0)
    {
        LoggerLog("Stats listener: WSAStartup failed (error %d) -- the STATUS display is unavailable for this run\n", wsaResult);
        return;
    }
    ...
    {
        LoggerLog("Stats listener: cannot create the listening socket (WSA error %d) -- the STATUS display is unavailable for this run\n",
                  WSAGetLastError());
        WSACleanup();
        return;
    }
    ...
    */
    ...
    if (setsockopt(listenSock, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse)) == SOCKET_ERROR)
        LoggerLog("Stats listener: warning -- could not set SO_REUSEADDR (WSA error %d); binding may fail right after a restart\n",
                  WSAGetLastError());
        ...
        */
        if (selectResult == SOCKET_ERROR)
        {
            ...
            if (selectErrorStreak == 1)
                LoggerLog("Stats listener: select failed (WSA error %d)\n", WSAGetLastError());

            if (selectErrorStreak >= 100)
            {
                LoggerLog("Stats listener: select failed %d times in a row -- giving up; the STATUS display is unavailable until the solver restarts\n",
                          selectErrorStreak);
                break;
            }
        ...
        if (client == INVALID_SOCKET)
        {
            ...
            */
            ...
            if (acceptFailures == 1 || acceptFailures % 100 == 0)
                LoggerLog("Stats listener: accept failed (WSA error %d; %d failure(s) so far)\n",
